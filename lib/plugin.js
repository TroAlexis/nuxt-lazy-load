...
let observer = null;
let attributes = ['srcset', 'poster', 'src'];
let elementsMap = {
  img: 'images',
  video: 'videos',
  audio: 'audios',
...
const setClasses = (el, type) => {
  let elementTagName = el.tagName.toLowerCase();
  let eventName = elementTagName === 'img' || elementTagName === 'picture' ? 'load' : 'loadeddata';
  if (['img', 'video'].includes(elementTagName)) {
    setLoadListeners(el, type, eventName)
  } else if (elementTagName === 'picture') {
    const img = el.querySelector('img');
    setLoadListeners(img, type, eventName)
  } else if (type === 'loaded' && options.loadedClass !== false) {
    el.classList.add(options.loadedClass)
  }
}

const setLoadListeners = (el, type, eventName) => {
